...
-- Force drop any existing views and their dependencies
...
-- Also check for any materialized views with the same names
DROP MATERIALIZED VIEW IF EXISTS public.enriched_artists CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.high_value_artists CASCADE;  
DROP MATERIALIZED VIEW IF EXISTS public.recent_discoveries CASCADE;

-- Now recreate the views with explicit SECURITY INVOKER (even though it's the default)
CREATE VIEW public.enriched_artists 
...
CREATE VIEW public.high_value_artists 
WITH (security_invoker = true) AS
SELECT 
  a.id,
  a.name,
...
CREATE VIEW public.recent_discoveries 
WITH (security_invoker = true) AS
SELECT 
  a.id,
  a.name,
...
-- Verify the security_invoker setting is correctly applied
SELECT 
  schemaname, 
  viewname, 
  viewowner,
...
AND viewname IN ('enriched_artists', 'high_value_artists', 'recent_discoveries');
